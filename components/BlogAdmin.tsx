      ...
      <div className="min-h-screen bg-white flex items-center justify-center px-4">
        ...
        >
          <div className="flex items-center gap-2 mb-8 justify-center">
            ...
            <span className="font-display font-bold text-xl tracking-tighter text-gray-900">RDS ADMIN</span>
          </div>
          <div className="bg-gray-50 border border-gray-200 rounded-sm p-6">
            <p className="text-sm text-gray-500 mb-4 font-mono">Enter admin password to continue</p>
            <input
              type="password"
              value={password}
              ...
              className="w-full bg-white border border-gray-300 rounded-sm px-4 py-3 text-gray-900 text-sm focus:outline-none focus:border-red-dot transition-colors mb-4"
              autoFocus
            />
            {loginError && <p className="text-red-500 text-xs mb-3">{loginError}</p>}
            <button
              onClick={handleLogin}
              className="w-full bg-red-dot text-white py-3 rounded-sm font-medium text-sm hover:bg-red-600 transition-colors flex items-center justify-center gap-2"
            >
          ...
          {installPrompt && (
            <button
              onClick={installApp}
              className="w-full mt-4 border border-gray-300 text-gray-500 py-3 rounded-sm font-mono text-xs hover:border-gray-900 hover:text-gray-900 transition-colors flex items-center justify-center gap-2"
            >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-5">
            <input
              ...
              className="w-full bg-transparent text-3xl md:text-4xl font-display font-bold text-gray-900 placeholder-gray-400 focus:outline-none border-b border-gray-200 pb-4"
            />
            ...
            <input
              ...
              className="w-full bg-transparent text-lg text-gray-600 placeholder-gray-400 focus:outline-none border-b border-gray-200 pb-3"
            />
            ...
            {/* Cover image preview */}
            {form.cover_image && (
              <div className="aspect-[21/9] overflow-hidden rounded-sm bg-gray-50 border border-gray-200">
...
![Image alt](https://image-url.com)"
                className="w-full bg-gray-50 border border-gray-200 rounded-sm px-5 py-4 text-gray-900 text-sm font-mono leading-relaxed placeholder-gray-400 focus:outline-none focus:border-red-dot transition-colors resize-y"
  ...
  return (
    <div className="min-h-screen bg-white pt-6 pb-20 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="flex items-center gap-3">
            {installPrompt && (
              <button
                onClick={installApp}
                className="border border-gray-300 text-gray-500 px-4 py-2 rounded-sm font-mono text-xs hover:border-gray-900 hover:text-gray-900 transition-colors flex items-center gap-2"
              >
            ...
            <button
              ...
              aria-label="Log out"
              className="border border-gray-300 text-gray-500 px-4 py-2 rounded-sm text-sm hover:border-gray-900 hover:text-gray-900 transition-colors flex items-center gap-2"
            >
              <LogOut size={14} />
            </button>
          </div>
        ...
        {/* Tab switcher */}
        <div className="flex gap-1 mb-8 bg-gray-50 border border-gray-200 rounded-sm p-1 w-fit">
          <button
            onClick={() => setAdminTab('blog')}
            className={`px-5 py-2 rounded-sm text-sm font-mono transition-colors flex items-center gap-2 ${
              adminTab === 'blog' ? 'bg-red-dot text-white' : 'text-gray-500 hover:text-gray-900'
            }`}
          >
          ...
          <button
            ...
            className={`px-5 py-2 rounded-sm text-sm font-mono transition-colors flex items-center gap-2 ${
              adminTab === 'portfolio' ? 'bg-red-dot text-white' : 'text-gray-500 hover:text-gray-900'
            }`}
          >
        ...
        <div className="grid grid-cols-3 gap-4 mb-10">
          <div className="bg-gray-50 border border-gray-200 rounded-sm p-5">
            <span className="text-2xl font-display font-bold text-gray-900">{posts.length}</span>
            <p className="text-xs text-gray-500 font-mono mt-1">Total Posts</p>
          </div>
          <div className="bg-gray-50 border border-gray-200 rounded-sm p-5">
            <span className="text-2xl font-display font-bold text-green-600">{posts.filter(p => p.published).length}</span>
            <p className="text-xs text-gray-500 font-mono mt-1">Published</p>
          </div>
          <div className="bg-gray-50 border border-gray-200 rounded-sm p-5">
            <span className="text-2xl font-display font-bold text-yellow-600">{posts.filter(p => !p.published).length}</span>
          ...
          <div className="space-y-3">
            {posts.map((post) => (
              <motion.div
                ...
                className="bg-gray-50 border border-gray-200 rounded-sm p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-gray-300 transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <h3 className="font-display font-medium text-gray-900 truncate">{post.title}</h3>
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className={`p-2 rounded-sm border transition-colors ${post.published ? 'border-green-300 text-green-600 hover:bg-green-50' : 'border-gray-300 text-gray-500 hover:text-yellow-600 hover:border-yellow-300'}`}
                  ...
                  <button
                    ...
                    className="p-2 rounded-sm border border-gray-300 text-gray-500 hover:text-gray-900 hover:border-gray-900 transition-colors"
                    title="Edit"
                  >
                    <Edit3 size={14} />
                  </button>
                  <button
                    ...
                    className="p-2 rounded-sm border border-gray-300 text-gray-500 hover:text-red-500 hover:border-red-300 transition-colors"
                    title="Delete"
                  >
                    <Trash2 size={14} />
                  </button>
